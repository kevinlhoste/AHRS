...
bool accGyroCalibrated=false;
float heading;
int headingBuffer[50];
     ...
     Serial.printf("[%u] get binary length: %u\r\n", num, length);
      ...
      if(frameType==0){
        ...
        //4 : right
          ...
          Serial.println("calibrate gyro acc");
          mpu.calibrateAccelGyro();
          accGyroCalibrated=true;
         }
        else if(command==6){
          Serial.println("calibrate magneto meter");
          mpu.calibrateMag();
          magCalibrated=true;
          timer0=millis();
         }
...
void loop(void) {
   if(millis()-timer0>=15 /*&& magCalibrated*/){
    mpu.update();
    getHeading();
    Serial.println("heading "+String(heading));
    headingAverage+=heading;
    samplesNb++;
    //webSocket.sendTXT(0,""+(String)heading);
    timer0=millis();
   }
  if(millis()-timer1>=1000){
    //int value=analogRead(36);
  //6.6/4095=0.00161172
  float batValue=analogRead(36)*0.00161172;
  Serial.println("bat  "+String(batValue));
    webSocket.sendTXT(0,"1 "+String(batValue));
    timer1=millis();
  }
 if(samplesNb==10){
  headingAverage=headingAverage/samplesNb;
  int intHeading=int(headingAverage);
  Serial.println("average heading  "+String(headingAverage));
  webSocket.sendTXT(0,"0 "+(String)intHeading);
  samplesNb=0;
  headingAverage=0;
 }
